  ...
  run(int)
  {
    try
    {
      ...
      cv::Mat_<float> depth(rows, cols);
      {
        ...
        cv::Mat_<cv::Vec3f> points3d_ini;
        cv::depthTo3d(depth_image, K, points3d_ini);
        // Fix the scale of each point to fall on the plane
        for (int y = 0; y < rows; ++y)
          for (int x = 0; x < cols; ++x)
          {
            cv::Vec3f &point = points3d_ini(y, x);
            point = point
                    * (-plane(0, 3)
                       / (point.val[0] * plane(0, 0) + point.val[1] * plane(0, 1) + point.val[2] * plane(0, 2)));
            depth(y, x) = point.val[2];
          }
      }
