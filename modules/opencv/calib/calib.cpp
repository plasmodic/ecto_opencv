...
namespace calib
{
  struct PingPongDetector
  {
    ...
    declare_params(tendrils& p)
    {
      ...
      p.declare<double>("dp", "The inverse ratio of the accumulator resolution to the image"
      ...
      p.declare<double>("param1", "The first method-specific parameter. in the case of CV_HOUGH_GRADIENT "
                        "it is the higher threshold of the two passed to Canny() edge "
                        "detector (the lower one will be twice smaller)",
                        200);
      p.declare<double>(
          "param2", "The second method-specific parameter. in the case of CV_HOUGH_GRADIENT "
    ...
    declare_io(const tendrils& params, tendrils& in, tendrils& out)
    {
      in.declare<cv::Mat>("image", "The grayscale image to search for a calibration pattern in.");
      out.declare<std::vector<cv::Vec3f> >("circles", "Detected circles, (x,y,radius).");
...
ECTO_CELL(calib, PingPongDetector, "PingPongDetector", "Detect 40 mm ping pong balls.");
